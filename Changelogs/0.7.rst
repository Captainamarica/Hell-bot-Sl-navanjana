...
* Updated Ravana's core. While installing unknown plugin you'll get a warning if replied plugin contains some harmfull codes. Though you can force install that plugin by using ``.install -f``. Use ``.install -f`` if you trust the provider or after checking the plugin codes.
...
* Added yet another waifu harem bot in autowaifu plugin `@NightVission <https://t.me/NightVission>`_. Simply send ``.adwaifu`` in a group you want to activate autowaifu.
